...
| 2026-07-19 | סימון ויזואלי ל-DEV — מחלקת `dev-env` על ה-body צובעת רקע/נאבבר/קלפים בגוון ירוק כהה. זיהוי dev = `import.meta.env.DEV` **או** hostname מקומי/`dev.*` (כי המכונה המקומית מגישה גם build דרך `vite preview`, שם DEV=false); הדומיין של פרוד (`app.safrasecure.uk`) לעולם לא נתפס. אומת בצילום מסך headless | `App.jsx`, `styles/admin.css` |
