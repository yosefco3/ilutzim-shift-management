...
| 2026-07-19 | סימון ויזואלי ל-DEV — תחת `vite dev` נוספת מחלקת `dev-env` על ה-body (גוון רקע/נאבבר ירקרק עדין) כדי להבדיל מיידית בין dev לפרוד; בילד פרודקשן (`import.meta.env.DEV=false`) אין שינוי | `App.jsx`, `styles/admin.css` |
